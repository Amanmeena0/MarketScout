...
def call_llm_with_backoff(llm, messages, max_retries=7, base_delay=4):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Gemini API rate limit or overload hit. Retrying in {delay:.2f} seconds... (attempt {attempt + 1}/{max_retries})")
            time.sleep(delay)
...
def get_llm():
    # Strict rate limiter for Gemini Free Tier (1 request every 10 seconds, no bursts)
